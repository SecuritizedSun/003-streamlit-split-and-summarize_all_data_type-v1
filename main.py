...
if uploaded_file is not None:
    ...
    if not openai_api_key:
        st.warning(
            "OpenAI API key is required to run the summarizer. Enter it above and re-run.",
            icon="⚠️",
        )
        st.stop()
    ...
    if not all(hasattr(doc, "page_content") for doc in splitted_documents):
        splitted_documents = [Document(page_content=doc) for doc in splitted_documents]
